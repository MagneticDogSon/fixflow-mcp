...
### 3. Create MD Card in `.agent/fixflow_kb/[category]/[KB_ID].md`
...
### 4. Update `.agent/fixflow_kb/index.json`
...
💾 .agent/fixflow_kb/terminal/WIN_TERM_003.md ✓
...
### 5. Upload to Remote DB (Supabase)

**Goal**: Store the KB card in the centralized database for team access, ensuring quality and preventing spam.

1.  **Server-Side Validation (Gatekeeper)**:
    Call the `validate_kb_card` tool with the generated markdown content.
    
    ```python
    # Example Tool Call
    response = call_tool("fixflow", "validate_kb_card", {"content": generated_markdown_content})
    if not response["valid"]:
        stop_and_fix()
    ```

    - **If Validation Passes (`✅ Validation PASSED`)**: Proceed to step 2.
    - **If Validation Fails (`❌ Validation FAILED`)**: Fix issues in the file content and retry validation. DO NOT UPLOAD invalid content.

2.  **Ensure Table Exists** (One-time setup):
    - Check if `fixflow_kb` table exists.
    - If not, execute the schema from `tech_kb_schema.sql` using `mcp_supabase-mcp-server_apply_migration`.

3.  **Insert/Update Data**:
    - Use `mcp_supabase-mcp-server_execute_sql` to insert the JSON data.
    - **Template** (Use `$mcp_kb_content$` tag to prevent SQL injection via `$$` in code):
    
    - **Secure RPC Call**:
    The database schema is now hidden. We call a secure function `save_kb_card` that handles the logic internally.
    ...
    SELECT save_kb_card(
        jsonb_build_object(
            ...
            'technologies', jsonb_build_array($val$[tech1]$val$, $val$[tech2]$val$),
            'complexity', [complexity],
            'criticality', $val$[criticality]$val$,
            'created_date', $val$[YYYY-MM-DD]$val$,
            'tags', jsonb_build_array($val$[tag1]$val$, $val$[tag2]$val$),
            'related_kb', jsonb_build_array($val$[RELATED_ID]$val$),
            'file_path', $val$[category]/[KB_ID].md$val$,
            'quick_summary', $val$[Summary]$val$,
            'quick_fix', $val$[Quick Fix]$val$,
            'diagnostic_time', $val$[Time]$val$,
            'fix_time', $val$[Time]$val$,
            'content', $mcp_content$[FULL_MARKDOWN_CONTENT]$mcp_content$
        )
    );
    ```
